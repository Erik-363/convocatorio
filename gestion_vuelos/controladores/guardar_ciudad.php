...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombre)) {
        ...
        $stmt = $conexion->prepare("SELECT id FROM ciudades WHERE nombre = ?");
        $stmt->bind_param("s", $nombre);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows > 0) {
            $_SESSION['error'] = "Esta ciudad ya existe.";
        } else {
            // Insertar nueva ciudad
            $insert_stmt = $conexion->prepare("INSERT INTO ciudades (nombre) VALUES (?)");
            $insert_stmt->bind_param("s", $nombre);
            
            if ($insert_stmt->execute()) {
                $_SESSION['mensaje'] = "Ciudad creada exitosamente.";
            } else {
